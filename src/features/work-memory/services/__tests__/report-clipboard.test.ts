...
describe('copyReportMarkdown', () => {
  it('writes markdown to clipboard without calling save logic', async () => {
    ...
    await copyReportMarkdown('# 本周回顾', { writeText });

    expect(writeText).toHaveBeenCalledWith('# 本周回顾');
  ...
  it('returns a friendly error when clipboard is unavailable', async () => {
    ...
    try {
      await expect(copyReportMarkdown('# 本周回顾')).rejects.toThrow('复制失败，请稍后重试。');
    } finally {
      Object.defineProperty(globalThis.navigator, 'clipboard', {
        configurable: true,
