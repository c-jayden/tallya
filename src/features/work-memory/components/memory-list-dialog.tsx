  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent
        ...
        className="tallya-dialog-content flex max-h-[calc(100vh-72px)] w-[min(620px,calc(100vw-48px))] max-w-none flex-col gap-0 overflow-hidden p-0 sm:max-w-[min(620px,calc(100vw-48px))]"
      >
        <DialogHeader className="shrink-0 px-6 pt-5 pb-4">
          {isDetailMode ? (
            <Button
              ...
              variant="ghost"
              size="sm"
              className="-ml-2 mb-3 h-7 cursor-pointer px-2 text-xs text-app-ink-muted hover:bg-app-surface-muted hover:text-app-ink focus-visible:bg-app-surface-muted focus-visible:text-app-ink"
              onClick={() => setSelectedMemory(null)}
            >
              ← 返回
            </Button>
          ) : null}
          <DialogTitle>{isDetailMode ? detailTitle : '工作记忆'}</DialogTitle>
          <DialogDescription>
            {isDetailMode
              ? '这是当天保存的正式工作记忆。'
              : '按时间倒序查看你已经沉淀的工作记录。'}
